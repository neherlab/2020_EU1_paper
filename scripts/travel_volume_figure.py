...
def import_figure(countries, roamers, country_to_iso, spain_frequency, cases_by_cw_per_capita):
    ...
    for country in countries:
        ...
        # this isn't actually needed as roaming data is corrected for missing data etc
        # this number reflects ratio of people flying/driving
        print(f"Flying/(Flying + driving) {country}: {roam_scale_factor}")
        ...
        axs[0].plot(roaming_date_points, np.array(list(roam_time_series.values()))/popsizes[country]*100000, label=country,
                c=country_styles[country]['c'], lw=2, ls=country_styles[country].get('ls', '-'))
